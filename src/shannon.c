...
void gen_shannon_bitstream(char* bitstream, char* input_str, int str_len, int sym_num,
                           node* sym_nodes) {
    int idx = 0;
    for (int i = 0; i < str_len; i++) {
        for (int j = 0; j < sym_num; j++) {
            if (input_str[i] == sym_nodes[j].symbol) {
                for (int k = 0; k < sym_nodes[j].sh_code_len; k++) {
                    bitstream[idx++] = sym_nodes[j].sh_code[k];
                }
            }
        }
    }
...
void decode_shannon_bitstream(char* bitstream, int sym_num, node* sym_nodes) {
    printf("Shannon decoded: ");
    int i = 0;
    while (bitstream[i] != '\0' && bitstream[i] != '\n') {
        for (int j = 0; j < sym_num; j++) {
            int match = 1;
            for (int k = 0; k < sym_nodes[j].sh_code_len; k++) {
                if (bitstream[i + k] != sym_nodes[j].sh_code[k]) {
                    match = 0;
                    break;
                }
            }
